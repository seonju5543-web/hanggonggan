...
/* 알맹이 — 날짜·기간·시각·연락처·주소·경로·금액 중 하나는 있어야 안내로서 쓸모가 있다 */
const HAS_SUBSTANCE = /\d{1,4}\s*[.\-/년]\s*\d{1,2}|\d{1,2}\s*월\s*\d{1,2}|\d{1,2}\s*[:：]\s*\d{2}|@|https?:\/\/|www\.|☎|\d{2,4}-\d{3,4}-\d{4}|→|＞|>|\d+\s?만\s?원|\d{3,}\s?원/;
/* 뒷말이 잘린 문장 — `…에 한하여 다음`, `…는 아래와 같이` 처럼 이어지다 끊긴 것 */
const DANGLING = /(다음|아래|같이|경우|위해|통해|관련|및|또는)\s*[.．]?\s*$/;
...
function extractFrom(text) {
  ...
  for (const p of parts) {
    ...
    /* 🔴 **알맹이가 없으면 담지 않는다** (2026-08-20 개발자 지적으로 전면 수정).
       그동안은 '신청기간·제출서류' 같은 낱말만 있으면 담았다. 그 결과 화면의
       '공고 원문 안내'에 이런 것들이 떴다:
         `제출서류 : 아래 두 서류 모두 제출` (무엇인지 없음)
         `② 단, 신청기간 이후 합격 발표자가 있는 경우에 한하여 다음` (문장이 잘림)
         `⑥ 전액장학 대상자 및 이중지원 대상자 제외` (안내가 아님)
       전수로 세어 보니 **374줄 중 266줄(71%)이 이런 조각**이었다.
       학생에게 쓸모가 있으려면 **날짜·연락처·주소·경로 같은 알맹이**가 실제로 들어 있어야 한다.
       없으면 담지 않는다 — 조각을 늘어놓느니 '원문 보기' 링크 하나가 낫다(원칙 8-1). */
    if (!HAS_SUBSTANCE.test(p)) continue;
    if (DANGLING.test(p)) continue;                        // 뒷말이 잘린 문장
...
function qualFromDocs(it) {
  /* 🔴 여기는 **1차(절 가르기)뿐이다. 2차(scoop)를 붙이지 말 것** (2026-09-04, 붙였다가 되돌렸다).
     ...
     정직하게 남기고 버튼을 열어 둔다. 푸른등대 기부장학금 2건도 같은 꼴이라 비어 있다 —
     저장된 요약표(`elig-1w5yb7-1.pdf`)가 기부처 22줄을 한 표에 담고 있고 줄마다 선발 대상이
     다르다(1행 `가수 윤하 : 기초과학(물리·천문학) 전공 대학생`). 22 는 그 PDF 를 세어 본 것이지
     registered.json 에 있는 숫자가 아니다.
