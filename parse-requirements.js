...
const HOME_PERSON = /^\s(?![^가-힣]*캠퍼스)([가-힣·]{0,8}?)(청소년|대학생|학생|주민|자녀|거주자)/;
...
/* 🔴 **넓힌 길에서는 이름이 확실할 때만 집는다** (2026-09-13 · 회귀를 만들고 잡았다).
   ...
   그래서 넓힌 길에서는 **꼬리말 앞이 두 글자 이상**인 이름과 시·도만 받는다.
...
const CLEAR_PLACE = /^[가-힣]{2,}(시|군|구)$/;
function homePersonAfterRegion(t) {
  for (const name of regionNamesIn(t)) {
    if (!REGIONS.includes(name) && !CLEAR_PLACE.test(name)) continue;
    let i = -1;
    while ((i = t.indexOf(name, i + 1)) >= 0) {
      ...
      const m = HOME_PERSON.exec(tail);
      if (m && !/캠퍼스|소재|대학교/.test(m[1])) return true;
    }
  }
  return false;
}
