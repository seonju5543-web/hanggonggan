...
const CACHE = 'handaejang-v204';  /* v204 — 대외활동·공모전 탭 (2026-09-25 · 노션 UI-34)
                                     ...
                                     v202 — 학교가 스스로 운영하는 장학 제도를 '교내'로 본다
                                     (2026-09-20 · match-engine 의 OWN_PROGRAMS). 경희대
                                     반영장학·우정장학 둘·경희꿈도전장학이 교외로 떠 있었다.
                                     ⚠️ 등록 단계에서만 쓴다 — 게시판 글 판정은 표식 그대로다.

                                     v201 — 처지(trait) 종류를 늘리고 넓혔다 (2026-09-19 ·
                                     물을 것이 있는 공고 18 → 27곳). 징계는 어느 칸에 있든
                                     결격이고(`TRAIT_DISQUALIFY`), 제외 칸에서는 그것만
                                     미달을 낸다. 물어서 답을 받은 성취는 ✓ 가 된다.
                                   v200 — 시작 화면이 정문 투어링(사진 3장 무작위 · 학교당 2.5초)이
                                     됐다 (2026-09-18 개발자 결정 · 노션 UI-3). 사진은 assets/gates/ —
                                     그림 규칙(캐시 우선)을 그대로 탄다.
                                   v199 — 「또는」 줄에서 처지 '아니요' 가 미달을 만들지
                                     않는다 (개발자 결정). `농어촌 출신 또는 기초생활수급자`
                                     가 기초생활수급자에게 미달로 떴다. 처지 축에만 · 맞은
                                     조건이 「또는」 반대편 갈래에 있을 때만 푼다.
                                   v198 — 학교가 제목에 `[교내]` 라고 **직접 적어 둔** 게시판
                                     글은 '교내' 칸에도 함께 뜬다 (개발자 지적 "실시간 공고 보니까
                                     교내에서 진행되는 거 있는데 교내탭에는 안 들어가져 있어").
                                     주최를 알아맞히는 것이 아니라 주최자가 적어 둔 글자를 읽는다 —
                                     표식 없는 글은 예전처럼 홈에만 남는다.
                                   v196 — '교내' 칸에는 **우리가 확인한 교내 공고만** 두고,
                                     학교 게시판 새 공고는 **홈**으로 옮겼다 (개발자 지시
                                     "어디에 게시되느냐가 아니라 어떤 재단이 주최하는가").
                                     누르면 게시판으로 나가는 동작은 그대로다.
                                   v195 — 실시간 공고 카드 첫 줄이 제목과 같은 말을 한다
                                     (개발자 지적 "교내공고에서 교내로"). `교내 공고` 로 못 박혀
                                     있어 제목이 `[공통][교외]` 인 공고가 교내로 떴다(27건 중 26건).
                                     누르면 학교 게시판으로 나가는 동작은 그대로다.
                                   v194 — '교내/교외'와 '주관 기관' 바로잡기 (개발자 지시 2026-09-18).
                                     교내는 표식이 있을 때만 · 게시한 학교를 주관 기관으로 적지 않는다.
                                     초안·제출처 안내·도우미가 '모름' 값을 학생에게 안 흘리게 같이 고침.
                                   v193 — 아이콘 버튼·뒤로 버튼의 **손가락 표적**을 44 로
                                     (개발자 결정 「충돌」 9 (b) · 2026-09-18). 보이는 모양은
                                     38×38 · 40×40 그대로고 안 보이는 덧판만 넓혔다.
                                   v192 — 네 갈래를 합쳤다:
                                     ① MY 「공통 서류정보」 아래 ~ 「서류 보관함」 빈 띠 줄이기 (v188 알림함 색·제출 서류 점 포함)
                                     ② 장학금 찾기 칩 둘 제거 — '우리 학교'를 '교내'에 합치고 '신청 가능만' 폐지
                                     ③ 자격 확인 도우미 — 못 읽은 자격을 공고 팝업 안에서 묻는다 (노션 AI-1)
                                     ④ 검색창 글자 17→16px · MY 「데이터 초기화」를 보관함 쪽으로 당김
                                   v197 — 자격 확인 도우미에 **처지(trait)** 를 붙였다: 프로필에 칸이
