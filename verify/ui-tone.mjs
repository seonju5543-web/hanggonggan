...
/* ── ⑤ 되돌아가지 않게 하는 톱니 (2026-09-10) ───────────────────────────────
   ...
   *"간격이 맞지 않은 부분이 많다"* 고 지적해 세로 간격을 네 단계(--sp-1~4)로 못 박았는데,
   ...
   ✅ 2026-09-10 페이스리프트에서 실제로 내렸다: 여백 521 → 20 · 색 95 → 76 · 없는 토큰 4 → 0.
      여백은 4px 격자로 스냅한 뒤 --space-* 토큰으로 바꿔서 줄였고(470개 선언),
      없는 토큰 4종은 새로 만들지 않고 **이미 있는 토큰**으로 이어서 없앴다.
      남은 20곳과 색 76곳은 대부분 값 하나짜리 특수 자리다 — 다음에 만질 때 더 내린다.
...
const CEILING = { space: 20, color: 76, ghost: 0 };
