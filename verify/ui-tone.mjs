...
/* ── ⑤ 되돌아가지 않게 하는 톱니 (2026-09-10) ───────────────────────────────
   ...
   *"간격이 맞지 않은 부분이 많다"* 고 지적해 세로 간격을 네 단계(--sp-1~4)로 못 박았는데,
   ...
   🔴 **2026-09-11 개발자 지시로 여백을 되돌렸다 → 천장을 523 으로 다시 올렸다.**
...
const CEILING = { space: 523, color: 76, ghost: 0 };
...
/* ── ⑥ 한국어 줄바꿈과 글꼴 (2026-09-10 신설 · 2026-09-11 범위 축소) ──────────
   ...
   남은 둘은 계속 지킨다:
   ① **index.html 이 Pretendard 를 계속 싣는다.** 이 앱 글자의 유일한 출처라,
      링크가 빠지면 학생 화면이 통째로 시스템 폰트로 떨어지고 아무도 모른다.
      🔴 실제로 2026-09-11 되돌리다 이 줄을 같이 지웠다가 이 검사로 잡았다.
   ② **한국어 줄바꿈(`keep-all`)을 끄지 않는다.** 이게 없어서 온보딩 제목이
      '알려주세 / 요' 로 끊겨 마지막 줄에 '요' 한 글자만 남아 있었다(실측).
      개발자가 페이스리프트에서 **이것 하나만 남기라고** 콕 집은 자리다.
   ③ 글자 크기 아홉 단계가 **존재하는지**만 본다(값은 안 본다) — 단계를 지우면
      그 크기를 쓰던 자리가 조용히 기본값으로 떨어진다. */
console.log('\n■ 한국어 줄바꿈과 글꼴 (2026-09-11 · 척도·서체 검사는 되돌림으로 뺐다)');
{
  ...
  /* ① 글자의 유일한 출처 — 이 줄이 빠지면 화면 글자가 통째로 바뀐다 */
