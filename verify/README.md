...
## 즉석 화면 확인 — 온보딩 건너뛰기 (2026-08-29)

개발자가 화면을 지적했을 때 **그 자리에서 눌러 보려고** 만든 것. 회귀 검사가 아니다.

1. 앱을 띄운다 — ⚠️ **포트를 8123 으로 고정하지 말 것.** 워크트리를 여러 개 쓰므로
   다른 세션의 서버를 재게 된다(2026-08-29 실사고 — `drive.js` 가 두 번 연속
   `ERRORS: none` 을 냈는데 전부 남의 코드였다). 드라이버는 전부 `PORT` 를 먼저 본다.
   ```
   python3 -m http.server 8129 &
   PORT=8129 node verify/drive.js
   ```
2. 브라우저 콘솔에 `verify/demo-profile.js` 내용을 붙여넣는다 → 홈 화면부터 시작한다.
   기준 학생은 `verify/what-shows.mjs` 와 **같은 값**이라 도구로 잰 것과 화면이 어긋나지 않는다.

🔴 앱 코드에 `?demo=1` 같은 뒷문을 만들지 않았다 — 그건 배포되는 앱에 남는다.
   (참고: `?e2e=1` 은 **아무 일도 안 한다.** app.js 에 그런 분기가 없다.)
