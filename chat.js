...
function chatAnswerHowTo(q) {
  ...
  /* 🔴 갈래를 **손으로 적지 않는다** (2026-09-21 코드 리뷰에서 잡힘).
     예전엔 네 줄을 여기 베껴 두고 "네 가지로 나눠 안내해요"라고 말했는데, data.js 에
     '접수 방법 미확인' 갈래가 생기자 **등록 68건 중 43건이 목록에 없는 답**을 받게 됐다.
     이제 라벨 원본(`SUBMIT_CHANNEL_LABEL`)에서 끌어오므로 갈래가 늘어도 어긋나지 않고,
     학생이 카드에서 보는 문구와 도우미가 말하는 문구가 **글자까지 같다**. */
  const kinds = chatSafe(() => Object.values(SUBMIT_CHANNEL_LABEL), []) || [];
  if (!kinds.length) {
    return { text: '접수 방법은 공고마다 달라요. 공고 이름을 함께 물어봐 주세요.' };
  }
  return {
    text: `접수 방법은 공고마다 달라요. 한대장은 ${kinds.length}가지로 나눠 안내해요.`,
    list: kinds,
