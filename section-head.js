...
/* `지원 혜택`은 실제 원문 이름표다(종근당고촌재단 `가. 지원혜택 : 주거비 전액지원`).
   맨 `혜택`을 넣으면 학교 홈 메뉴의 `의료기관 진료혜택`이 걸리므로 `지원`을 붙여 좁힌다. */
const AMOUNT_WORD = /(장학\s?금액|지급\s?금액|지원\s?금액|수혜\s?금액|지급\s?액|지급\s?내역|장학금?\s?(지급|내용|혜택|종류)|지원\s?(내용|혜택)|장학\s?내용)/;
...
/* 🔴 `금 액 :` 도 이름표다 (2026-08-27 전수 조사 — 한국외대 공고 3건이 이 꼴이었다).
     `3 ) 금 액 : 250 만 원`  `나 . 금 액 : 1 인 150 만 원`  `나 . 금 액 : 1 인 1,449,399 원`
   ⚠️ 콜론을 반드시 요구한다 — 표 머리(`장학금액 ( 원 )`)나 본문의 `납부 금액` 은 콜론이 없다.
   ⚠️ `등록금액:` 은 일부러 뺀다(학생이 **내는** 돈이라 장학금액이 아니다). */
const AMOUNT_BARE = /^[^:：]{0,12}(장학\s?금|(?<!등록)금\s?액|지원\s?금)\s*[:：]/;
...
/* 🔴 두 갈래를 갈라 둔다 — 세기가 다르다 (2026-08-27 전수 재조사에서 갈랐다).
   전에는 한 덩어리였고, **줄 전체**에 댔다. 그래서 이름표는 멀쩡한데 뒤에 붙은 내용에
   그 낱말이 있으면 진짜 금액 줄이 통째로 죽었다 — 실제 원문 4건이 그렇게 0원이었다:
     `2. 장학금액: 학기당 생활보조비 100만원 지원(결격사유가 없는 한 1년간 지급)`
     `1. 장학금액: 생활비성 장학금 300만원(휴학 등 사고시 제외)`
     `3. 장학금액 및 지급기간: 수업료전액, 학업보조비 300만원/학기`
   이름표는 콜론 앞이고, 콜론 뒤는 그 절의 **내용**이다(isHead 가 이미 그렇게 본다). */
/* `유의사항`·`주의사항` 절은 **금액을 말하는 절이 아니다.** 거기서 금액을 주우면
   상한·금지 조항이 장학금액으로 둔갑한다 — 실제로 그랬다(2026-08-27 전수 대조):
     `6. 장학금 지급 관련 유의사항 - … 다른 장학금과 합산하여 등록금의 100%를 초과하여
      지급할 수 없습니다.` → 이 공고가 **등록금 100%를 준다**고 읽혔다.
   `장학금 지급`이 AMOUNT_WORD 에 있고 나머지(`관련유의사항`)가 딱 6자라 길이 관문을
   통과해 버린다. 그래서 낱말로 못 박는다. */
const AMOUNT_NEVER = /결격|제외|유의\s?사항|주의\s?사항/;          // 이름표에 있으면 무조건 아니다
const AMOUNT_WHEN  = /지급\s?(시기|시점|일정|방법|기간|절차|계좌)/;   // 금액 낱말이 따로 있으면 져 준다
...
function isAmountHead(line) {
  ...
  const t = headText(line);
  /* 이름표(콜론 앞)만 본다 — 콜론 뒤는 이 절의 내용이지 이름표가 아니다.
     콜론이 없으면 줄 전체가 이름표다(`<교내 장학금 지급 결격사유>` 는 여기서 걸린다). */
  const label = t.split(/[:：]/)[0];
  if (AMOUNT_NEVER.test(label)) return false;
  /* `장학금 지급시기` 는 때를 말하는 머리글이라 아니고, `장학금액 및 지급기간` 은
     이름표가 금액을 이미 말하고 있으니 금액이다. 때 낱말을 뺀 나머지로 가른다. */
  if (AMOUNT_WHEN.test(label) && !AMOUNT_WORD.test(label.replace(AMOUNT_WHEN, ' '))) return false;
