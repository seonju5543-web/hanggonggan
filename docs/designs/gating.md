...
| 매칭 공개 (오후 결정) | **매주 화·금 밤 9시(KST)에 한 번** 모아 맺는다 — Date Drop·위밋 방식. 5분 예약은 만료 정리만 |
| 글에서 대화 열기 (오후 결정) | **관심 → 글쓴이 수락 → 방**. 앱은 방을 직접 여는 RPC 를 못 부른다(권한 회수) · 거절은 상대에게 알리지 않는다 |
...
- 5분마다 만료 정리 · 화·금 21시(KST · `DROP_CRON`)에 `pairRequests()`(순수 함수) → `gating_commit_match` RPC 로 원자 확정.
...
8. **관심 없이 방을 여는 RPC 를 학생에게 다시 주지 말 것** (2026-09-21 결정) — `gating_open_room` 은 수락 RPC 안에서만 돈다.
9. **`DROP_CRON` 과 wrangler.toml 의 둘째 cron 은 글자까지 같아야 한다** — 워커가 그 문자열로 '정리만/맺기' 를 가른다(test-collector 가 대조).
...
**개발자가 같은 날 저녁 정한 둘** (2절 표에 적었다 · 구현 완료): ① 매칭은 화·금 밤 9시에 한 번 — `DROP_CRON`(worker.js)과 wrangler.toml 둘째 cron 이 한 쌍이고 `scheduled` 가 `event.cron` 으로 가른다 · 화면은 `gtNextDrop()` 으로 다음 공개 시각을 적는다. ② 관심 → 수락 → 방 — 표 `gating_interests`(트리거가 스냅숏·규칙 · 하루 10건) · RPC `gating_accept_interest`/`gating_decline_interest`(글쓴이만) · `gating_open_room` 은 학생 권한에서 회수. 관문: verify-gating [5] · verify-gating-server '5분 예약은 정리만' · test-collector 「과팅 탭」 끝 절.
