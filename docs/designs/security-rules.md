...
| 판정에 쓰는 프로필을 **우리 사본에서 읽는다**(요청 본문의 프로필은 안 읽는다) | `server/apply/send-log.mjs` 의 `loadProfile` |
...
✅ **조작한 프로필로 자격을 맞히는 것도 2026-09-26 에 막았다.** 판정은 `profiles` 표의 우리 사본으로만 한다.
🔴 **여기서 칸을 쪼갤 필요는 없었다** — 엔진은 프로필 **객체**를 받으므로 JSONB 를 그대로 읽어 넘기면 된다.
   고문 Q6 의 '컬럼으로 쪼개라'가 필요해지는 것은 **서버가 프로필로 질의할 때**(예: "이 공고에 맞는 학생 찾기")이고,
   우리는 그런 질의를 하지 않는다. 쪼개는 값은 아직 없고 값은 있다(앱 구조가 바뀔 때마다 DB 도 고쳐야 한다 —
   `supabase/migrations/0001_profiles.sql` 주석).

⚠️ 남은 천장 둘
- **서버 사본은 일부러 덜 담긴다** — 특별자격·처지는 동의했을 때만 있다. 그래서 동의를 안 켠 학생의
  특별자격 공고는 **막는다.** 🔴 그때 "자격이 없다"가 아니라 **"우리가 확인할 수 없다"** 로 말한다
  (`code: 'sensitive_off'`) — 있는데 없다고 하면 거짓말이다.
- **사본이 폰보다 낡을 수 있다**(저장 뒤 2초 미룬 올리기). 서버에 프로필이 없으면 422 가 아니라
  **409 `no_profile`** 로 돌려보내, 앱이 한 번 올리고 다시 부를 수 있게 한다.
