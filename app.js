...
/* 카드 제목에서 기관명 접두어를 지운다 (2026-09-10 페이스리프트).
   한 카드에 기관명이 두 번 나오고 있었다 — 제목 '재단법인 연수큰재장학재단 학업우수장학생'
...
function cardTitle(sch) {
  ...
  if (!prov) return name;
  const bare = (t) => t.replace(/^(재단법인|사단법인|학교법인|의료법인|\(재\)|\(사\)|재\)|사\))\s*/, '').trim();
  for (const p of [prov, bare(prov)]) {
...
function schCard(sch, result, { compact = false, fit = 0, fd = null } = {}) {
  ...
  return `
    ...
    <button class="sch-card" data-detail="${sch.id}">
      ...
      <div class="sch-top">
        <span class="sch-org">${esc(sch.type)} · ${esc(sch.provider)}</span>
        ${badge}
      </div>
...
function renderExplore() {
  ...
  /* 🔴 **학교 게시판 글은 '교내' 칸에서 함께 나온다** (2026-09-17 개발자 지시:
     "교내와 우리학교 탭에서 같은 공고가 계속 발견되고 있는데 우리학교 탭을 없애 교내로 합병").
     2026-09-12(노션 UI-16)에 제 칸('우리 학교')으로 뺐던 것을 되돌린 것이 아니다 — 칸만
     합치고 **구역은 남긴다**: 게시판 글은 여전히 우리가 등록한 공고와 다른 종류라
     (적합도도, 마감 판정도, 신청 준비도 없다 — 원문 링크만 준다) 아래쪽 '우리 학교
     실시간 공고' 머리말 밑에 모여 나온다. 섞어서 한 목록으로 정렬하면 적합도·마감이
     없는 카드가 정렬 기준을 통째로 무력화한다.
     🔴 **정렬 버튼은 이제 늘 보인다** — 감췄던 이유는 그 칸에 게시판 글'만' 있어서였다.
        교내 칸에는 등록 공고(적합도·마감이 있는 카드)가 함께 있으니 감추면 학생이
        고른 기준을 쓸 수 없게 된다.
  ...
  /* 실시간 공고는 **'교내' 칸에서만** 나온다 (2026-09-17 개발자 지시로 '우리 학교' 칸을
     여기에 합쳤다). '전체'에는 두지 않는다 — 노션 UI-16 이 칸을 가른 이유가 그대로 남아
     있다: 등록 공고 카드 수십 장 아래에 붙으면 스크롤 끝까지 가야 보인다. */
  const noticesHtml = inSchoolTab ? liveNoticesHtml(exploreQuery) : '';
  $('#live-notices').innerHTML = noticesHtml;
  ...
  /* 🔴 **아래에 실시간 공고 구역이 있으면 '없어요'를 적지 않는다** (2026-09-17).
     교내 칸은 등록 공고와 게시판 글이 함께 사는 자리라, 등록분이 0건일 때 예전 문구를
     그대로 쓰면 **'없어요' 라고 적어 놓고 그 바로 아래에 공고를 늘어놓는** 화면이 된다.
     그 구역은 제 머리말과 제 빈 상태 문구('아직 …게시판 연결 전이거나 새 공고가 없어요')를
     이미 갖고 있으므로, 여기서는 비워 두고 그쪽이 말하게 한다. */
      ...
      <div class="sch-top">
        ${/* 🔴 **`교내 공고` 로 못 박혀 있었다** (2026-09-18 개발자 지적: "교내공고에서 교내로").
             ...
             🔴 판정을 여기서 새로 만들지 않는다 — `match-engine.js` 의 `noticeKind` 하나다
                ...
                실시간 구역에서 다른 말을 한다). */ ''}
        ${/* 🔴 뒤에 `게시판` 을 붙인다 — 이 자리는 목록 카드에서 **'누가 주는가'**를 적는
             칸이라(`교외 · (재)수림재단`), 학교 이름만 두면 `교외 · 한국외국어대학교` 가 되어
             "교외 장학금인데 주는 곳이 한국외대" 로 읽힌다. 이 글은 그 학교가 주는 것이
             아니라 그 학교 **게시판에 올라온** 것이다(주관 기관은 우리가 모른다). */ ''}
        <span class="sch-org">${esc(noticeKind(n.title))} · ${esc(n.school)}${n.campus ? ' ' + esc(n.campus) : ''} 게시판</span>
