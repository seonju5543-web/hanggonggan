...
description: "한대장(한국 대학교 장학금) 모바일 PWA 의 디자인 시스템. 흰색에 베이지 한 방울을 탄 밝은 바탕(#fefdfc) 위에 먹빛 글자(#17191c), 그리고 채도를 낮춘 잉크블루(#27508f) **하나**만을 강조색으로 쓴다. 동작(누를 수 있음)은 남색 잉크(#1e2b42)가 맡고, 뜻을 가진 색은 역할 셋(마감·검수 전·선정)에 색조 둘(빨강·초록 — 2026-09-11 개발자 결정 12번)이고 전부 채도 60% 아래다. 본문도 제목도 Pretendard 한 벌이고 제목은 굵기·자간으로만 구분한다(2026-09-11 개발자 지시로 SUIT·SUITE 에서 되돌림) — 숫자(금액·남은 날·적합도)가 화면의 주인공이라 tabular-nums 로 자리를 고정한다. 카드는 흰 면 위의 얇은 선(hairline)으로 가르고, 그림자는 떠 있는 것(시트·토스트·당겨서 새로고침 표시·마스코트·정렬 팝오버)과 **주 버튼**에만 준다 — 하단 내비는 그림자가 아니라 **안쪽 실선 하나**로 뜬다 — 2026-09-12 에 코드를 이 문장에 맞췄다(아바타·히어로 패널·온보딩 로고와 기능 카드에 남아 있던 그림자를 걷어 홈 5 → 3 · 온보딩 5 → 1. 관문 verify-interactions '그림자는 떠 있는 것에만'). 참고안 둘 — Linear(어두운 마케팅 캔버스)와 Apple(사진이 주인공인 갤러리) — 에서 **구조**(단일 강조색 · 면의 사다리 · 얇은 선 · 4px 격자 · 알약은 '고르는 것'의 모양 · 44px 표적 · 이름 붙인 컴포넌트)를 받되, **값**(어두운 바탕·라벤더·Inter·SF Pro·17px 본문)은 이 앱의 기존 결정(2026-08-31 팔레트 · 2026-09-11 서체·크기 되돌림)을 지킨다. 값을 바꾸는 항목은 맨 아래 「참고안 둘과 충돌 · 컨펌 대기」에 모아 두었다."
...
| 터치 표적 44px | 탭·입력·정렬·**아이콘 버튼**(2026-09-18 적용)은 통과 · 글자 버튼 21 · 필터 칩 41.5 는 아직 미달 — 아래 「충돌」 9번 |
...
- **Accent** (`{colors.accent}` #27508f): **이 앱의 유일한 유채색.** 쓰는 자리는 넷 — 눌린 탭(`bottom-nav-selected`) · 화면 제목 밑 강조선(**글자 폭만큼** — 2026-09-13 개발자 지시로 28px 고정에서 바뀌었다) · 링크 버튼(`button-link`) · 눌린 필터 칩의 **2px 테두리**(2026-09-11 부터 칠하지 않는다 · Apple 방식). 🔴 형광 파랑(#2f6bff)으로 되돌리지 말 것 — 2026-08-31 개발자 지시. Apple 파랑(#0066cc)도 같은 이유로 받지 않는다 — 그 색 하나가 앱을 '흔한 화면'으로 만든다.
...
| `{typography.input}` | 16px (`--t-input`) | 400 | — | — | 검색창 · 도우미 입력칸 — **척도가 아니라 기능값**(2026-09-17) |
...
| `icon-button`(알림 · 설정) | 보이는 **38×38** · 닿는 넓이 **44×44** | **통과 (2026-09-18 · 9 (b) 적용)** — 안 보이는 덧판 `.icon-btn::after` |
| `button-link`("전체 보기" · "더보기") | **58×21 · 40×21** | 44 미만 — 글자 높이 그대로 |
| `sub-back`(안쪽 화면 뒤로 · 8곳) | 보이는 **40×40** · 닿는 넓이 **44×44** | **통과 (2026-09-18 · 9 (b) 와 같이)** — 같은 덧판 |

남은 둘(`filter-chip` 41.5 · `button-link` 21)은 아직 판정만 적는다 — 개발자가 9번에서 고르신 것은 **아이콘 버튼 하나**다.
고칠 때는 표적만 키우고 글자 크기는 그대로 둔다. 🔴 **넓히는 방법이 둘이고 고르는 기준이 있다**:
· 보이는 면이 **없는** 것(`button-link`)은 `.my-edit-hint` 처럼 여백(padding) + 음수 여백 — 눈에 안 띈다.
· 보이는 면이 **있는** 것(`icon-button` 의 hover 판 · `filter-chip` 의 알약)은 여백을 키우면 **그 면이 같이 커져**
  승인받은 모양이 바뀐다. 그래서 아이콘은 안 보이는 덧판(`::after`)으로 넓혔다 — 칩도 같은 방법이어야 한다.
...
| 나머지 | 유지 | 1·2·3·4·5·6·8·10·11·14·15·17 은 건드리지 않았다(개발자 지시). **9 는 2026-09-18 에 (b) 로 적용됐다**(아래 표). 21~23 중 21 은 물었고 아직 답을 안 받았다 |
...
| 9 | **터치 표적** | CTA ≥ 40 · 입력 ≥ 44 | **전부 ≥ 44** | **아이콘 (b) 적용(2026-09-18)** — 보이는 38 그대로 · 닿는 넓이 44 | 적용됨. `.icon-btn::after` 덧판 — padding 을 키우면 음수 여백으로 맞춰 둔 정렬이 어긋난다. 관문 `verify-settings.js` '아이콘 버튼의 손가락 표적'(귀퉁이 넷을 실제로 짚는다) |
...
**정해 주면 이렇게 진행한다**: 아직 열린 것은 **5·6·10·14·15·17·19·21** 이다 — 번호마다 (a)/(b) 또는 "유지"만 적어 주면 된다.
**5·6·14·21** 은 전부 덜어내거나 한 줄 고치는 방향이라 한 번에 하는 것이 자연스럽다. **10** 은 앱 인상이 바뀌므로
실제 앱에 임시로 적용해 폰으로 본 뒤 정한다. **21** 은 360px 폰을 쓰는 학생이 지금도 보고 있는 화면이라 먼저 해도 된다.
1·2·3·4·8·11·22·23 은 개발자 결정이 이미 있거나 참고안 둘이 정반대라 다시 묻지 않는다. 7·9·12·13·16·18·20 은 적용됐다.
