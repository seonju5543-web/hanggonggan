...
| 선택 칩을 칠하지 않는다 | **2026-09-11 적용(7번)** — `{colors.surface-2}` 바탕 + 강조색 글자 + `{colors.accent-line}` 선 |
| 포커스 링은 2px · 또렷하게 | **2026-09-11 적용(13번)** — `--ring` 2px · 강조색 50% |
...
- **Accent** (`{colors.accent}` #27508f): **이 앱의 유일한 유채색.** 쓰는 자리는 다섯 — 눌린 탭(`bottom-nav-selected`) · 구획 머리의 건수(`.list-group-n`) · 화면 제목 밑 28px 강조선 · 링크 버튼(`button-link`) · 눌린 필터 칩의 **글자와 선**(2026-09-11 부터 칠하지 않는다). 🔴 형광 파랑(#2f6bff)으로 되돌리지 말 것 — 2026-08-31 개발자 지시. Apple 파랑(#0066cc)도 같은 이유로 받지 않는다 — 그 색 하나가 앱을 '흔한 화면'으로 만든다.
...
| 포커스 | `--ring` 0 0 0 2px 강조색 50% (13번 · 2026-09-11 적용) | 입력칸 · 버튼 |
...
- **눌림**: `.btn:active { transform: scale(0.98) }` — 큰 버튼만. 칩·아이콘 버튼·카드는 배경색만 바뀐다. Apple 은 **누를 수 있는 것 전부에 `scale(0.95)` 하나**를 건다 — 「충돌」 20번.
...
- 기본: `{colors.surface-1}` 바탕 · 1.5px 잉크 10% 선 · `{colors.ink-muted}` 글자 550. 선택(**2026-09-11 적용 · 7번**): `{colors.surface-2}` 바탕 + `{colors.accent}` 글자 + `{colors.accent-line}` 선 · 그림자 없음 — Linear 방식. `{rounded.pill}` · 8.5px 15px · 실측 높이 41px.
- ⚠️ Apple 은 같은 자리에 흰 바탕 + 2px 파란 테두리(`configurator-option-chip-selected`)를 쓴다 — 지금 적용된 Linear 방식과 **둘 다 칠하지 않는다**는 점은 같다. 「충돌」 7번.
...
**`text-input`** — `{colors.surface-1}` · 1px 잉크 10% 테두리 · `{rounded.sm}` 13px · 16px · `{typography.body}`. 포커스: 테두리 `{colors.accent}` + `--ring`(2px 50%). 오류(`:user-invalid` · 16번 적용): 테두리 `{colors.semantic-danger}` + 포커스 링도 같은 색, 문구는 그대로. 최소 높이 56px(통과). 검색 입력만 알약(Apple 의 `search-input` 과 같다). 체크박스는 앱 공용 그림을 써야 보인다(전역 `appearance:none`).
...
| 나머지 | 유지 | 1·2·3·4·5·6·8·9·10·11·14·15·17 은 건드리지 않았다(개발자 지시). 20~23 은 그 뒤에 나온 것이라 **아직 묻지 않았다** |
...
| 7 | **선택 칩** | surface-2 로 한 단 올림 | 흰 바탕 + **2px 파란 테두리** | **Linear 방식 적용(2026-09-11)** | 적용됨. Apple 방식(흰 바탕 · 2px 테두리)은 대조판에 나란히 두었다 — 지금 것이 더 조용해 **유지 권함** |
...
| 13 | **포커스 링** | 2px · 50% | 2px solid · 흐리지 않음 | **2px · 50% 적용(2026-09-11)** | 적용됨. Apple 의 불투명 2px 와 차이가 미세해 **유지** |
...
| # | 항목 | Apple | 지금 앱 | 제안 |
|---|---|---|---|---|
| 20 | **누름 반응** | 모든 버튼 `scale(0.95)` 하나 | `.btn:active` 0.98 · 칩·아이콘은 반응 없음 | **바꾸는 쪽 권함.** 값(0.95/0.97)보다 **전부에 거는 것**이 요점. "정적이다"(2026-09-09 지적)와 닿는다 |
...
**정해 주면 이렇게 진행한다**: 아직 열린 것은 **5·6·9·10·14·15·17·19·20·21** 이다 — 번호마다 (a)/(b) 또는 "유지"만 적어 주면 된다.
**5·6·9·14·20·21** 은 전부 덜어내거나 한 줄 고치는 방향이라 한 번에 하는 것이 자연스럽다. **10** 은 앱 인상이 바뀌므로
실제 앱에 임시로 적용해 폰으로 본 뒤 정한다. **21** 은 360px 폰을 쓰는 학생이 지금도 보고 있는 화면이라 먼저 해도 된다.
1·2·3·4·8·11·22·23 은 개발자 결정이 이미 있거나 참고안 둘이 정반대라 다시 묻지 않는다. 7·12·13·16·18 은 적용됐다.
