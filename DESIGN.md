...
| press | 오버레이 16% · scale 없음 | `scale(0.95)` 전부(20번 · 개발자 결정) · 행(`.my-menu-item`·알림 설정 줄·FAQ)은 면 없이 **글자만 강조색**(2026-09-12 개발자 지시 '누를 때 회색 판 제거' · 4114줄) | 다르다 — 개발자 결정 |
...
### 다크 규칙이 밝은 화면에 새던 것 (2026-09-11 발견 · 2026-09-12 코드 리뷰로 수리 방향 정정)

`:root:not([data-theme="light"]) .x { … }` 는 **data-theme 이 없으면 언제나 참**이다. 이 앱은 어디에서도 data-theme 을
달지 않으므로(index.html·app.js·boot.js grep 0건) 초안 도우미(essay-*)의 다크 규칙 여섯은 '시스템이 다크일 때'가 아니라
'늘' 켜져 있었고, 밝은 화면에서 `.essay-rule-danger` 가 연어색(#ff8a7a), `.esc-badge.esc-ok` 가 진초록 바탕(#1f4a2e)으로
떠 있었다(브라우저 실측 · colorScheme light/dark 둘 다 같은 값).
첫 수리는 그 선택자를 `@media (prefers-color-scheme: dark)` 안으로 옮기는 것이었는데 **코드 리뷰가 잡았다** — 그러면 폰이
다크인 학생에게만 흰 시트 위에 진초록 배지가 뜬다. 그것은 2026-08-24 개발자 지시(`style.css` '적합도 색깔' 절 머리말 —
*"다크모드 규칙을 넣지 말 것. 앱 전체가 밝은 한 벌이라 부분 다크는 흰 바탕 위에 어두운 조각만 띄운다"*)가 금지한 바로 그
모습이다. 그래서 **여섯 규칙을 지웠다** — 밝은 값(`#c0392b` · 초록 옅은 바탕)만 남고, 시스템이 다크여도 같은 화면이다.
관문: `ui-tone` ⑧ '다크 규칙이 없다'(미디어 블록 · `[data-theme="dark"]` · `:root:not([data-theme="light"])` 전부 0 ·
red-green 확인). 앱 전체 다크모드를 만드는 날(1번) 이 관문을 함께 걷어낸다.
...
- **눌림**: **`scale(0.95)` 하나** — 버튼·칩·아이콘·정렬·링크·ghost 전부(20번 · 2026-09-11 적용 · Apple 값). 전엔 `.btn` 0.98 과 상호작용 묶음 0.975 로 둘이었고 정렬·링크는 빠져 있었다. ⚠️ 이 문서 첫 판의 "칩·아이콘은 반응 없음"은 틀렸다(202줄만 보고 단정) — 0.975 로 반응하고 있었다. 카드는 버튼이 아니라 면(`surface-2`)으로만 답하고, 행(`.my-menu-item`)은 **2026-09-12 개발자 지시로 회색 판을 뺐다** — 누르면 글자만 강조색이 된다(`.my-menu-item:not(.danger):active` · 4114줄).
...
- 다크 규칙을 **일부에만** — 앱은 밝은 한 벌이라 부분 다크는 다크 폰에서 흰 바탕 위 어두운 조각으로 튄다(2026-08-24 지시). 게다가 `:root:not([data-theme="light"])` 는 data-theme 이 없는 이 앱에서 언제나 참이라 밝은 화면에도 샌다(2026-09-11 실사고 · 관문 `ui-tone` ⑧ 은 다크 규칙 0줄을 센다).
...
- **다크 모드가 없다 — 그리고 부분 다크도 넣지 않는다**(2026-08-24 지시). 폰 설정이 다크인 학생에게도 밝게 뜬다. `essay-*` 에 있던 다크 규칙 여섯은 밝은 화면에도 새고 있어 2026-09-12 에 지웠다(위 「다크 규칙이 밝은 화면에 새던 것」) — 관문 `ui-tone` ⑧ 이 다크 규칙 0줄을 지킨다. Apple 문서도 밝은 판만 기록했고, Vapor 만 다크 램프를 따로 둔다(1번 · 별도 작업).
...
| 20 | **적용(오후)** | 누름 반응 `scale(0.95)` 를 버튼·칩·아이콘·정렬·링크·ghost 전부에. 행은 면으로만 → 2026-09-12 회색 판도 뺌(글자만 강조색) |
...
| # | 항목 | Apple | 지금 앱 | 제안 |
|---|---|---|---|---|
| 20 | **누름 반응** | 모든 버튼 `scale(0.95)` 하나 | **`scale(0.95)` 전부 적용(2026-09-11 오후)** — 전엔 0.98/0.975 둘 · 정렬·링크는 빠짐 | 적용됨. 행(`.my-menu-item`)은 2026-09-12 부터 면 없이 글자만 강조색 |
...
1. ✅ **다크 규칙 여섯이 밝은 화면에 새던 것** — 지웠다(위 「Vapor 참고안」 절 · 관문 ⑧ · 2026-09-12 코드 리뷰 반영).
...
8. 신청내역 진척도 4단계가 **팔레트 밖 색조 넷**을 순서대로 쓴다(`.app-step-s0~s3` 2793~2800줄 · 남색 #22304d → 보라 #5a3f78 → 자주 #9c3f63 → 빨강 #cf4436 · 막대와 단계 글자 둘 다). 12번(색조 둘) 뒤에 남은 자리라 강조색 한 단계 + 회색으로 줄이면 유채색 천장이 8 준다. '대기' 단계의 주황(`--orange`)은 12번 뒤 빨강과 같아 '위험'으로 읽힌다 — 함께 볼 것.
