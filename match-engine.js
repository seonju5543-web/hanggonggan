...
/* 🔴 뜻이 바뀌었다 (2026-09-21) — 예전엔 '요건은 읽었으나 **하나도** 충족을 확인 못 한 경우'
   였는데, 그 상태는 이제 `unread`(FIT_UNREAD)로 나간다. 하나도 못 맞힌 것을 '적합 15%'라고
   말하던 것이 울산 남구·전남 장학금을 강서구 학생에게 '적합'으로 띄운 원인이었다.
   지금 이 값이 맡는 자리는 **하나 이상 맞혔지만 비율이 아주 낮을 때의 바닥**이다
   (예: 10개 중 1개 = 10% → 15). 미달 확정(FIT_MIN)과는 여전히 구분한다. */
const FIT_FLOOR = 15;
...
function fitDetail(sch, p) {
  ...
  /* 🔴 **하나도 판정하지 못했으면 '적합'이 아니라 '모른다'** (2026-09-21).
     예전에는 `fails` 가 비면 곧바로 적합이라, 요건을 **0개 판정**하고도 카드가 '적합'이라고
     말했다. 실측으로 걸린 것: 울산 남구 장학금(`남구에 주민등록`)과 전남 장학금
     (`전남광주통합특별시에 주소`)이 **강서구 사는 학생에게 적합**으로 떴다 — 요건 3개 중
     met 0, 3개 전부 '모른다'였는데도 그랬다.
     거주 요건은 일부러 'fail' 을 안 내는 자리가 많다(시·도 불일치·예외 문구 — 틀린 미달이
     못 받는 것보다 나쁘므로). 그 관대함이 여기서 **'틀린 안심'으로 뒤집히고 있었다.**
     🔴 `unread` 로 낸다 — 위 `!items.length` 와 뜻이 같다("우리는 이 공고의 자격을 모른다").
        화면은 이미 그 값을 '자격 미확인' 배지로 그리고 퍼센트도 FIT_UNREAD 로 맞춘다.
        여기서 내지 않고 app.js 에서 따로 판정하면 **퍼센트와 배지가 다른 말을 한다**
        (감사의 '일관성 축'이 보는 것이 그것이다).
     ⚠️ `total === 0` 은 위에서 이미 걸러졌고, 하나라도 맞힌 공고는 그대로 지나간다. */
  if (total > 0 && met === 0) return { pct: FIT_UNREAD, unread: true, met, total, unknown, fails: [] };
