...
switch (action) {
  ...
  case 'register': {
    ...
    const entry = {
      ...
      /* 🔴 **기본값은 로봇과 같아야 한다** (2026-09-18 코드 리뷰).
         로봇이 '교내는 표식이 있을 때만 · 주관 기관은 모르면 모른다고' 로 바뀌었는데 여기만
         옛 기본값(교내 · `○○ 게시 공고`)으로 남으면, 관리자가 공고 하나를 등록하는 순간
         새 관문이 빨간불이 되고 **그 실행의 자동 등록분이 통째로 되돌려진다**(revert-auto). */
      type: patch.type === '교내' ? '교내' : '교외',
      provider: patch.provider || '주관 기관 원문 확인',
