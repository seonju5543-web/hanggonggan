...
/* 2026-09-15 넓힘 둘 (AI 가 포스터·PDF 에서 옮겨 온 실제 줄에서 걸렸다):
   · `2.5백만원`(푸른등대 기부장학금 요약표) — 소수점을 못 읽어 `5백만원` 이 됐다(2배 과장).
   · `12,420천원`(한미 첨단분야 청년교류) — 예산 문서의 천원 단위. 못 읽어 미확인이었다.
   천원은 만원 뒤에 둔다(`5천만원` 이 먼저 잡혀야 한다) · 5천원 같은 수수료는 MIN_WON 이 거른다. */
var WON_PATTERNS = [
  { re: /(\d[\d,]*(?:\.\d+)?)\s*억\s*(\d[\d,]*)?\s*만?\s*원?/, mul: 100000000 },
...
/* 🔴 **숫자 뒤의 1 은 '1인당'이 아니다** (2026-09-17 · 노션 UI-12).
   `1\s*명` 이 `311명`·`21명`·`101명` 안의 `1명` 에 걸려, 예산표의 합계 행
   `지급금액 계 311명 421백만원` 이 **1인당 4억 2천만원**으로 읽혔다. 게다가 이 오탐은
   바로 아래 총액 방어선을 **건너뛰게** 만든다(1인당이 있으면 총액 검사를 안 한다).
   ⚠️ 저장분 전수로 재 보니 판정이 갈리는 조각은 **1개**(`대학생 11명` — 없애려던 그 오탐)
      뿐이고 저장된 본문에는 0개다. 진짜 `1인당`·`1명`·`명당` 은 그대로 잡힌다. */
var PERSON_RE = /(1\s*인\s*당|인\s*당|(^|[^0-9])1\s*인|(^|[^0-9])1\s*명|명\s*당|인\s*기준|(^|[^0-9])1인)/;
/* 🔴 낱말 **순서가 뒤집힌 것**도 총액이다 (2026-08-30 — 한국장학재단 등록분에서 걸렸다).
   `○ 예산 총 5억원 1천만원 범위 내 ○ 최소 50만원 최대 400만원` 이 `총 예산` 과 안 맞아
   **5억원이 1인당 장학금**으로 잡혔다. 그리고 `단체` 수상금은 애초에 1인당이 아니다:
   `대회수상 장학금: 50만원(개인)~1700만원(단체)`. 둘 다 넣는다.
   ⚠️ 넓힐 때는 **기존 금액이 안 바뀌는지 전수로 확인할 것** — 축소는 안전하지만
      진짜 금액까지 0 이 되면 그것도 틀린 것이다. */
/* 🔴 **표의 합계 행도 총액이다** (2026-09-17 · 노션 UI-12).
   위 낱말들은 전부 '총'·'사업비' 같은 **말**을 찾는데, 요강의 예산표는 그런 말 없이
   `지급금액 계 | 311명 | 421백만원` 처럼 **합계 행**으로 적는다. 표는 한 줄에 한 칸씩
   펼쳐져 오므로 금액 절이 통째로 삼킨다 — 울산연구원 공고문 HWP 가 실제로 그 꼴이고,
   첨부를 금액 경로에 붙이자마자 **1인당 4억 2천만원**이 나왔다(115명 몫 180백만원도 함께).
   ⚠️ 저장분 전수로 재 보니 이 규칙이 **지금 금액이 있는 27건 중 0건**을 잃는다.
   ⚠️ 글자가 깨져도 잡아야 한다(`계 311浵ࡦ명`) — HWP 추출이 흔히 그렇게 낸다.
      그래서 인원 앞에 깨진 글자 세 자까지 허용한다.
   ⚠️ `선발 계획 30명`·`모집인원 10명` 은 걸리면 안 된다 — 인원만으로는 총액 신호가 아니다.
   관문: `test-collector.mjs` 금액 절 '표의 합계 행'. */
var TOTAL_TABLE = /(^|[\s.·:])계\s*[\d,]+\s*\S{0,3}명|(금액|장학금|지원금)\s*계(\s|$)|합\s*계|소\s*계|총\s*계/;
...
function perPersonWon(blockLines) {
  ...
  for (var i = 0; i < arr.length; i++) {
    /* ⚠️ 숫자 사이의 쉼표(`1,000,000원`)는 조각 경계가 아니다 (2026-09-15) — 거기서 가르면
       `1인당 최대 1` / `000` / `000원` 이 돼 1인당 줄의 금액을 못 읽고 뒷줄로 넘어간다(세종이도 실측). */
    var segs = String(arr[i]).split(/;|,(?!\d{3})|·|▷|&middot;/);
...
/* 🔴 **인원 × 예산 표도 총액이다** (2026-09-15 — 울산연구원 하반기 공고문 HWP 에서 걸렸다).
   원문의 지급 계획표가 글자로 뽑히면 한 줄에 이렇게 붙는다:
       `지급금액 계 311명 421백만원 우수장학금 대학생 115명 180백만원`
   `총·예산·사업비` 같은 낱말이 하나도 없어 TOTAL_RE 를 그대로 지나가고, 1인당 최대액은
   다른 줄(`1,500천원`)에 있어 PERSON_RE 도 못 본다 → **1억 8천만원이 1인당 장학금**이 됐다.
   ⚠️ 표는 칸마다 줄이 갈라져 나온다 — HWP 본문 글자는 `115명` 과 `180백만원` 이 **다른 줄**이다.
      그래서 줄이 아니라 **블록**으로 본다: 블록 안에 사람 수(두 자리 이상 `명`)가 있고,
      금액이 적힌 줄 어디에도 `1인당·각·명당` 표시가 없으면 그 블록은 '몇 명에게 얼마를
      나눈다'는 예산표다. 읽지 않는다.
   ⚠️ 한 자리 인원(`선발인원 3명, 각 100만원`)은 안 막는다 — 소수 인원 공고는 1인당 금액을
      적는 것이 보통이라 예산표일 가능성이 낮다. 넓힐 때는 기존 절대액이 안 줄어드는지
...
function amountFrom(lines) {
  ...
  if (!perWon && (TOTAL_RE.test(body) || TOTAL_TABLE.test(body))) {
    return { kind: 'unknown', value: 0, ratio: 0, min: 0, max: 0, raw: body.slice(0, 200) };
  }
  /* 인원 × 금액 예산 줄뿐이면 같은 이유로 읽지 않는다 (HEADCOUNT_RE 주석) */
  if (!perWon && budgetOnly(blk.lines)) {
    return { kind: 'unknown', value: 0, ratio: 0, min: 0, max: 0, raw: body.slice(0, 200) };
  }
