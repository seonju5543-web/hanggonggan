...
5. 「인스타 토큰 수명 확인」 을 수동 실행한다 — **이 단계를 건너뛰지 말 것**
   (시크릿 화면이 채워져 있는 건 증명이 아니다. 물어봐야 안다)
   🔴 **초록불만 보면 안 된다** (2026-09-12 인수인계 중 확인) — 시크릿이 **아예 없어도**
      이 워크플로는 초록불이다(`state=none` 으로 정상 종료한다). 로그를 열어
      `state=ok` · `살아 있음 · 우리가 아는 한 N일 남음` 까지 봐야 증명이다.
   🔴 **이 확인은 `IG_ACCESS_TOKEN` 만 본다 — `IG_USER_ID` 는 안 본다.** 아이디가 틀려도
      여기는 초록이고, 처음 「게시」 를 누를 때 가서야 드러난다. 아이디는 3번에서
      `/me?fields=user_id` 가 돌려준 값을 그대로 넣으면 틀릴 일이 없다.
      둘 다 맞는지 한 번에 보려면 관리자 화면 「인스타」 › 게시 대기에서 **예행연습**을
      돌린다(`publish.mjs` 는 기본이 예행연습이라 실제로 안 올라간다).
